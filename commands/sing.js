	...
	try {
		ytdl(handleReply.link[event.body - 1])
			...
			.on("close", () => {
				...
				else return api.sendMessage({ body: "Chúc các bạn thưởng thức bản nhạc này 😇",attachment: createReadStream(__dirname + `/cache/${handleReply.link[event.body - 1]}.m4a`)}, event.threadID, () => unlinkSync(__dirname + `/cache/${handleReply.link[event.body - 1]}.m4a`), event.messageID)
			})
